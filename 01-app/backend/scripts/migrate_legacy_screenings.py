"""Replace legacy rules screenings with frozen-classifier screenings.
...
def migrate(*, apply: bool) -> tuple[int, int, list[dict]]:
    init_db()
    ...
    try:
        for row in rows:
            ...
            if not isinstance(old, dict) or not is_legacy(old):
                skipped += 1
                continue
...
def main() -> None:
    ...
    migrated, skipped, manifest = migrate(apply=args.apply)
    if args.manifest:
