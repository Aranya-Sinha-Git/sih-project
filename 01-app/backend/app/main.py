...
def readiness_payload() -> tuple[dict[str, Any], bool]:
    ...
    payload = {"status": "ready" if ready else "not_ready", "model_mode": "Supervised screening classifier", "model_status": metadata["status"], "model_version": metadata["model_version"], "lsr_model_status": domain["lsr_status"], "lsr_model_version": domain["lsr_version"], "lsr_supported_rule_ids": domain.get("supported_rule_ids", []), "lsr_unavailable_rule_ids": domain.get("unavailable_rule_ids", []), "checks": checks, "llm_configured": False, "runtime_generative_llm_calls": False, "database": get_database().kind}
