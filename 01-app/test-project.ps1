...
try {
  ...
  & (Join-Path $projectRoot 'backend\.venv\Scripts\python.exe') -m pytest -q (Join-Path $projectRoot '..\03-training\ml\sif_v0_1\tests\test_pipeline.py') -k 'human_evaluator or frozen_metadata or calibration'
  if ($LASTEXITCODE -ne 0) { throw "Frozen evaluator checks failed (exit code $LASTEXITCODE)." }
  npm run build
