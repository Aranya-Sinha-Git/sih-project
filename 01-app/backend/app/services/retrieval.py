...
class RetrievalService:
    ...
    def reference_evidence(self, query: str, limit: int = 5) -> list[dict[str, Any]]:
        ranked = _rank(query, [f"{item.get('title','')} {' '.join(item.get('aliases', []))} {item.get('text','')}" for item in self.references], limit, 0.08)
        return [{"evidence_id": item["evidence_id"], "rule": item.get("rule"), "title": item.get("title"), "excerpt": item.get("text"), "narrative_spans": _spans(query, item.get("text", "")), "relevance_score": round(score, 4), "retrieval_method": "tfidf_cosine", "corpus_version": self.corpus_version, "citation": {"publisher": item.get("publisher"), "source_url": item.get("source_url"), "document_version": item.get("document_version"), "page_section": item.get("page_section")}} for index, score in ranked for item in [self.references[index]]]
